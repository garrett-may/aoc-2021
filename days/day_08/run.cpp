...
struct digit_t{
    constexpr std::size_t count_on() const noexcept{
        return std::accumulate(values.cbegin(), values.cend(), 0);
    }
    // check to see whether one digit contains another digit,
    // segment-wise
    // for example, '9' contains '4', because:
    //
    //  --              
    // |  |                |  |
    //  --     contains     --
    ...
    //  --
    //
    // if one looks at the segments, each segment that is in
    // '4' is also in '9'    
    constexpr bool contains(digit_t const& digit) const noexcept{
        for(std::size_t i = 0; i < values.size(); ++i){
            if(digit.values[i] && !values[i]){
                return false;
            }
        }
        return true;
    }
    std::array<bool, 7> values;
};

constexpr digit_t parse_digit(char const* str, char const** endptr) noexcept{
    digit_t digit{};
    for(; *str != ' ' && *str != '\0'; ++str){
        digit.values[*str - 'a'] = true;
    }
    if(endptr){
        *endptr = str;
    }
...
struct display_t{
    std::array<digit_t, 10> patterns;
    std::array<digit_t, 4> output;
};

constexpr display_t parse_display(char const* str) noexcept{
    display_t display{};
    ...
    while(*str != '\0'){
        ...
        auto digit = parse_digit(str, &str);
        if(!flip){
            display.patterns[counter] = digit;
        } else{
            display.output[counter] = digit;
        }
        ++counter;
    }
    return display;
...
constexpr auto find_unique_digits_in_outputs(std::array<T, size> const& input) noexcept{
    std::array<display_t, size> displays{};
    for(std::size_t i = 0; i < size; ++i){
        displays[i] = parse_display(input[i]);
    }
    return std::accumulate(displays.cbegin(), displays.cend(), 0, [](auto init, auto const& display) constexpr noexcept{
        return init + std::accumulate(display.output.cbegin(), display.output.cend(), 0, [](auto init, auto const& digit) constexpr noexcept{
            switch(digit.count_on()){
                case 2:
                case 3:
                case 4:
                case 7:
...
constexpr auto interpret_display(display_t const& display) noexcept{
    display_t fixed_display{};
    for(auto const& pattern : display.patterns){
        switch(pattern.count_on()){
            case 2:
                fixed_display.patterns[1] = pattern;
                break;
            case 3:
                fixed_display.patterns[7] = pattern;
                break;
            case 4:
                fixed_display.patterns[4] = pattern;
                break;
            case 7:
                fixed_display.patterns[8] = pattern;
                break;
            default:
                break;
        }
    ...
    for(auto const& pattern : display.patterns){
        switch(pattern.count_on()){
            case 6:
                if(pattern.contains(fixed_display.patterns[7])){
                    if(pattern.contains(fixed_display.patterns[4])){
                        fixed_display.patterns[9] = pattern;
                    } else{
                        fixed_display.patterns[0] = pattern;
                    }
                } else{
                    fixed_display.patterns[6] = pattern;
                }
                break;
            default:
                break;
    ...
    for(auto const& pattern : display.patterns){
        switch(pattern.count_on()){
            case 5:
                if(pattern.contains(fixed_display.patterns[7])){
                    fixed_display.patterns[3] = pattern; 
                } else{
                    if(fixed_display.patterns[6].contains(pattern)){
                        fixed_display.patterns[5] = pattern;
                    } else{
                        fixed_display.patterns[2] = pattern;
                    }
                }
                break;
            default:
    ...
    std::size_t value = 0;
    for(auto const& digit : display.output){
        for(std::size_t i = 0; i < fixed_display.patterns.size(); ++i){
            if(digit.values == fixed_display.patterns[i].values){
                value = value * 10 + i;
                break;
            }
        }
...
template<typename T, std::size_t size>
constexpr auto count_outputs(std::array<T, size> const& input) noexcept{
    std::size_t sum = 0;
    for(std::size_t i = 0; i < size; ++i){
        auto display = parse_display(input[i]);
        auto value = interpret_display(display);
        sum += value;
    }
    return sum;
}
...
constexpr static std::size_t part_1 = find_unique_digits_in_outputs(input);
constexpr static std::size_t part_2 = count_outputs(input);
